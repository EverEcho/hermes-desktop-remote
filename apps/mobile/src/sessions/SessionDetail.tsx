...
import { MobileSurface } from '@/ui/primitives'
  ...
  return (
    <div className="h-full flex flex-col relative bg-(--ui-bg-chrome)">
      ...
      >
        ...
        {awaitingResponse && (
          <div className="flex items-center gap-2 py-1.5 text-(--conversation-scaffold-meta)">
      ...
      {userScrolledUp.current && (
        <button
          className="absolute bottom-20 right-3 size-7 rounded-[4px] border border-(--stroke-nous) shadow-(--shadow-nous) bg-(--ui-bg-elevated) grid place-items-center text-(--ui-text-secondary) z-10"
...
function MessageRow({ message }: { message: MobileMessage }) {
  ...
  if (message.role === 'tool') {
    return (
      <div className="flex items-center gap-1.5 py-0.5 text-(--conversation-scaffold-meta)">
  ...
  if (message.role === 'user') {
    return (
      <div className="flex justify-end mb-2">
        <div className="max-w-[85%] rounded-[var(--btn-radius)] bg-(--ui-chat-bubble-background) border border-(--ui-stroke-tertiary) px-3 py-2">
          <MarkdownContent content={message.content} className="text-(--conversation-text-font-size) leading-[var(--conversation-line-height)]" />
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="mb-2">
      {message.error ? (
        <div className="flex items-start gap-1.5 text-(--ui-red)">
      ...
      {message.reasoning && (
        <details className="mt-1">
          ...
          <p className="mt-0.5 text-(--conversation-tool-font-size) text-(--conversation-scaffold-meta) whitespace-pre-wrap max-h-32 overflow-y-auto no-scrollbar border-l border-(--ui-stroke-tertiary) pl-2">
            {message.reasoning}
          </p>
        </details>
      )}
...
function ToolCallRow({ tc }: { tc: NonNullable<MobileMessage['toolCalls']>[number] }) {
  return (
    <details className="mt-1">
      <summary className="flex items-center gap-1.5 cursor-pointer select-none py-0.5">
        <span
          className={cn(
      ...
      <MobileSurface className="mt-1">
        {tc.summary && (
          <p className="text-(--conversation-tool-font-size) text-(--conversation-scaffold-meta)">
            {tc.summary}
          </p>
        )}
        {tc.inlineDiff && (
          <pre className="mt-1 text-(--conversation-tool-font-size) font-mono overflow-x-auto whitespace-pre-wrap max-h-32 overflow-y-auto no-scrollbar text-(--conversation-scaffold-text)">
            {tc.inlineDiff}
          </pre>
        )}
      </MobileSurface>
