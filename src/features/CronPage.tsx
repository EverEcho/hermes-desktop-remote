...
function CronEditor({ job, onClose, onSaved }: { job?: CronJob; onClose: () => void; onSaved: (job: CronJob) => void }) {
  ...
  const save = async () => {
    ...
    try {
      const saved = job
        ? await api.updateCronJob(job.id, { name: name.trim(), schedule: cron.trim(), prompt: prompt.trim(), ...(delivery ? { deliver: delivery } : {}) })
        : await api.createCronJob({ name: name.trim(), schedule: cron.trim(), prompt: prompt.trim(), ...(delivery ? { deliver: delivery } : {}) })
      onSaved(saved)
    } catch {
      setError(t.cron.saveFailed)
    } finally {
